...
let evensOnly = function(num){
  if(num % 2 === 0){
    return true
  }
}

let evenNumbers = mixedNumbers.filter(evensOnly)
...
let populationSum = function(runningTotal, curElement){
  return runningTotal + curElement
}
let totalPopulation = populations.reduce(populationSum)
...
let orderTotals = orders.map((order) => {return (order.price * order.tax)+ order.price})
...
let bob = function(purchase){
  if(purchase.owner === "Bob"){
    return true
  }
}

let bobPurchases = purchases.filter(bob)

let addPrices = (accumulator, curr) => {
  return accumulator + curr.price
}

let bobsTotal = bobPurchases.reduce(addPrices, 0)
